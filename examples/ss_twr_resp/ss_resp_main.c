...
// VOXL
#define DSPAL_SPI_TX_BUFFER_LEN 500
...
#define RX_BUF_LEN 1024 // VOXL: changed to 1024
...
{ 
  // // wake up the device from DEEPSLEEP
  // uint8 buffer[500];
  // int len = 500; // needs to be large enough to keep the SPI CSn pin low for at least 500us, which depends on SPI speed
  ...
  {};
  uint32 test = dwt_read32bitreg(0x24);
  printf("0x24: %" PRIu32 "\n", test);

  printf("Status reg: %" PRIu32 "\n", status_reg);
  ...
  if (status_reg & SYS_STATUS_RXFCG)
  {
    ...
    {
      // make sure the frame fits in VOXL's SPI buffer 
      // printf("frame length: %d \n", frame_len);
      if (frame_len > DSPAL_SPI_TX_BUFFER_LEN ) {
        uint16_t offset = 0;
        int numReads = ceil(((float) frame_len)/DSPAL_SPI_TX_BUFFER_LEN);
        int i;
        // printf("num reads %d \n", numReads);
        for (i = 0; i < numReads; ++i) {
          int rxBufferIndex = i * DSPAL_SPI_TX_BUFFER_LEN; 
          uint32 readLen = DSPAL_SPI_TX_BUFFER_LEN;

          if (i == (numReads-1)) {
            // last read; read all the rest
            readLen = frame_len - (i * DSPAL_SPI_TX_BUFFER_LEN);
          };
          // printf("Reading rx buffer %d \n", i);
          // printf("Readlen %d \n", readLen);
          dwt_readrxdata(&rx_buffer[rxBufferIndex], readLen, 0);
        };
      } else {
        // read whole frame at once
        // printf("Reading rx buffer \n");
    ...
    * As the sequence number field of the frame is not relevant, it is cleared to simplify the validation of the frame. */
    ...
    /* Reset RX to properly reinitialise LDE operation. */
    // dwt_rxreset(); // TODO ROBIN: This causes the IC to either go to a fail-state or DEEPSLEEP (SYS STATUS return 0xFFFFFFFF)
  }
