...
int main(void)
{
  ...
  // /* Reset DW1000 */
  reset_DW1000(); 
  printf("reset DW1000 done \n");
  // dwt_softreset();

  /* Init VOXL SPI */
  voxl_spi_init(VOXL_SPI_BUS, VOXL_SPI_BUS_MODE, VOXL_SPI_FREQ);
  printf("VOXL SPI init done \n");

  // /* Set SPI clock to 2MHz */
  // port_set_dw1000_slowrate();			
  // printf("setting DW1000 SPI freq to 2 MHz done \n");
  ...
  // set Clock PLL lock detect tune to 1, to see if there is an error 
  uint8 pllDetectTuneOn = 1 << 2;
  dwt_writetodevice(0x24, 0, 1, &pllDetectTuneOn);
  ...
  // uint8 dataA[10] = { 1,1,0,3,1,9,9,4 };
