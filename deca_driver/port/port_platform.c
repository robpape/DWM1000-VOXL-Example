...
{ 
  ...
  // printf("port platform; idatalength = %d\n", idatalength);

  voxl_spi_transfer(VOXL_SPI_BUS, idatabuf, idatalength, itempbuf, idatalength);
  
  p1=itempbuf;

  memcpy(readBuffer, p1, readlength); // get the first "readLength" bytes
...
{
  // configure pin as output and set to low to reset
  voxl_gpio_init(DW1000_RST,GPIO_DIRECTION_OUTPUT,0);
  voxl_gpio_write(DW1000_RST, 0);

  // og implementation delays here for 2ms...

  // set pin back to input
  voxl_gpio_init(DW1000_RST,GPIO_DIRECTION_INPUT,0);

  // og implementation delays for 2ms again...
}
